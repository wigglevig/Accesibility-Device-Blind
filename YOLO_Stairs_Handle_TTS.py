...
def get_terminal_color(label):
    mapping = {
        ...
        "traffic light": WHITE,
        "stairs": RED,
        "door_handle": (255, 0, 255)  # Example: assign magenta for door handle
    }
...
def get_detection_color(label):
    # For stairs and door_handle we use fixed colors.
    if label.lower() == "stairs":
        return (0, 0, 255)
    if label.lower() == "door_handle":
        return (255, 0, 255)  # Magenta for door handle
    if label not in class_colors:
        class_colors[label] = random.choice(vibrant_colors)
    return class_colors[label]

# -------------------------
# Define STAIRS_COLOR (for stairs model)
# -------------------------
...
# -------------------------
# Asynchronous Full-Frame Depth Inference for a Selected Detection
# -------------------------
...
print("Press a digit key (1-9) for that object's depth; press 'd' for center depth; press 'v' for verbal description; press 'V' for item counts; press 'q' to quit.")
