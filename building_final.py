...
##################################
# Helper: Draw Text with Background (for annotations)
##################################
...
##################################
# Dashboard Settings (Live Feed & Bottom Panels)
##################################
TOP_WIDTH, TOP_HEIGHT = 1280, 720     # Live feed resolution
BOTTOM_WIDTH, BOTTOM_HEIGHT = 640, 360 # Two panels side-by-side
...
#cap = cv2.VideoCapture("http://192.168.1.103:8080/?action=stream")
...
# For hazard alerts (optional) we could add a cooldown timer.
hazard_last_alert = 0
...
while True:
    ...
    # Run YOLO inference every 2 frames
    if frame_count % 2 == 0:
    ...
    # Build HUD: Top panel is live feed; bottom panel (if depth info available) is two side-by-side panels
    ...
    # Check for hazard alerts (optional)
    current_time = time.time()
    for i, label in enumerate(all_classes):
        if label.lower() in hazard_labels:
            # For hazard, check if the detection is near the bottom center of the frame
    ...
    # Manual trigger: Digit keys for depth query
    if key >= ord('1') and key <= ord('9'):
        ...
        # Verbal summary with counts and positions
        if all_boxes:
            summary = {}
            for i, box in enumerate(all_boxes):
                x1, y1, x2, y2 = map(int, box)
                ...
                if x_center < TOP_WIDTH / 3:
                    horiz = "left"
                elif x_center > 2 * TOP_WIDTH / 3:
                    horiz = "right"
                else:
                    horiz = "center"
                # Determine vertical position
                if y_center < TOP_HEIGHT / 3:
                    vert = "top"
                elif y_center > 2 * TOP_HEIGHT / 3:
                    vert = "bottom"
                else:
                    vert = "middle"
                pos = f"{vert} {horiz}"
