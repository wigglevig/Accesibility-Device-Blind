...
def get_terminal_color(label):
    mapping = {
        ...
        "traffic light": WHITE,
        "stairs": RED,
        "door_handle": (255, 0, 255)  # Magenta for door handle
    }
...
##################################
# Dashboard Settings (16:9 Live Feed on Top; Bottom: Two Panels)
##################################
TOP_WIDTH, TOP_HEIGHT = 1280, 720     # Live feed resolution
BOTTOM_WIDTH, BOTTOM_HEIGHT = 640, 360 # Two panels side-by-side (total 1280 width)
...
##################################
# Speech Recognition (Triggered on 'c' press)
##################################
import speech_recognition as sr
...
# This function now automatically selects the detection by matching detected labels.
...
##################################
# Video Capture (Raspberry Pi Stream)
##################################
...
print("  'c': Record a distance query (automatically fetch distance)")
print("  'q': Quit")
...
while True:
    ...
    if frame_count % 2 == 0:
        ...
        results_door_handle = door_handle_model.predict(source=frame, device=device)[0]
        all_boxes = []
        all_classes = []
        all_scores = []
        ...
        if results_stairs.boxes is not None:
            for box in results_stairs.boxes:
                ...
                all_scores.append(float(box.conf.cpu().numpy()[0]))
        if results_door_handle.boxes is not None:
            for box in results_door_handle.boxes:
                all_boxes.append(box.xyxy.cpu().numpy()[0])
                all_classes.append("door_handle")
                all_scores.append(float(box.conf.cpu().numpy()[0]))
    ...
    # Create HUD: Top panel is live feed; bottom panel (if depth available) is two side-by-side panels.
    ...
    # Manual trigger using digit keys remains available
    if key >= ord('1') and key <= ord('9'):
        ...
        # Invert depth mapping: close objects yield high values for COLORMAP_JET so that they appear red.
        ...
        if all_boxes:
            unique_items = list(dict.fromkeys(all_classes))
        ...
        # When 'c' is pressed, record a voice command and automatically trigger depth query
    ...
    # Optionally, uncomment the next line to monitor frame processing time.
